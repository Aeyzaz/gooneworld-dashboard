...
import { servicePath, apiPath } from 'Constants/defaultValues'
...
class Listings extends Component {
    ...
    constructor(props) {
      ...
      this.state = {
        ...
        orderOptions:[
          {column: "id",label: "IdListing"}],
          selectedOrderOption:  {column: "id",label: "IdListing"},
    ...
    dataListRender() {
      ...
      let query = {
        params: {
          filter: {
            ...
            order:selectedOrderOption.column+ ' DESC',
            limit:selectedPageSize,
            skip: ((currentPage-1)*selectedPageSize) // (1-1)x10 = 0 , OK ok
          }
        }
      }
      ...
      axios.get(apiUrl_Goone+'/api/ManualListings/count?',{
        params:{
      ...
      .then(res=> {
        ...
        this.setState({
          totalPage : Math.ceil(res.data.count/selectedPageSize),
          totalItemCount : res.data.count
        });

        axios.get(apiUrl_Goone+'/api/ManualListings?',query)
            .then(res => {
            console.log('data',res.data);
            //this.state.items.push.apply(this.state.items,res.data);
    ...
    handleDeleteListing = (idlisting) => {
      
      //let data.id = idlisting;
      let data = {
          id : idlisting
      };
      ...
      axios.delete(API_URL, axiosConfigObject)
      .then(res => {
        this.dataListRender();
      });
          ...
          <div className="disable-text-selection">
            ...
            <Row>
              {this.state.items.map(product => {
                ...
                //console.log("miimage ",imagelisting);

                var imagelisting = "";
                  //console.log("producto b ", product.images[0]);

                  if(typeof product.images === 'undefined'){
                    imagelisting = 'http://www.esmeth.com/wp-content/uploads/2014/11/coming_soon_thumb.png';
                  }else{
                    if(product.images.length > 0){
                      imagelisting = product.images[0].link;  
                    }else{
                      imagelisting = 'http://www.esmeth.com/wp-content/uploads/2014/11/coming_soon_thumb.png';
                    }
                  }
                ...
                if (this.state.displayMode === "imagelist") {
                  ...
                  var typelink = "";
                  var typelink_photos = "listingphotos/"+product.id;
                  
                  /*switch(product.typedata){
                    case "Improved Commercial/Industrial":
                    typelink = "listingedit_imp/"+product.id;
                    break;
                    case "Residential Income":
                    typelink = "listingedit_res/"+product.id;
                    break;
                    case "Business Opportunity":
                    typelink = "listingedit_buss/"+product.id;
                    break;
                    case "Commercial/Business/Agricultural/Industrial/Land":
                    typelink = "listingedit_com/"+product.id;
                    break;
                  }*/

                  switch(product.transaction_type){
                    case "Sale":
                      typelink = "listingedit_sale/"+product.id;
                    break;
                    case "Lease":
                      typelink = "listingedit_rent/"+product.id;
                    break;
                    default:
                      typelink = "listingedit_sale/"+product.id;
                    break;
                      ...
                      >
                        ...
                        >
                          <div className="position-relative">
                            ...
                            <Badge color={product.active == "1" ? "success" : "danger"} pill>
                          ...
                          <CardBody>
                            <Row>
                              ...
                              <Colxx xxs="12" className="mb-3">
                                <CardSubtitle>{product.address}</CardSubtitle>
                                <CardText className="text-muted text-small mb-0 font-weight-light">
                                  {product.city} {product.state} {product.zip}
                                </CardText>
                                <CardText className="text-muted text-small mb-0 font-weight-light">
                                  {product.typeofproperty =="" ? " ": product.typeofproperty}
                                </CardText>
                                <CardText className="text-muted text-small mb-0 font-weight-light">
                                  {product.price}
                                </CardText>
                              ...
                              <Colxx xss="12">
                                <Button color="secondary" href={typelink_photos} className="default mb-2">
                                  {/*<IntlMessages id="Photos" />*/}
                                  Photos
                                </Button>
                                {" "}
                                <Button color="secondary" href={typelink} className="default mb-2">
                                  {/*<IntlMessages id="Edit" />*/}
                                  Edit
                                </Button>
                                {" "}
                                <Button color="danger" className="default mb-2" onClick={() => this.handleDeleteListing(product.id)}>
